...
func genTypes(dir string, cfg *config.Config, api *spec.ApiSpec) error {
	...
	apiFileName := parseFileName(api.Info.FileName)
	typeFilename, err := format.FileNamingFormat(cfg.NamingFormat, apiFileName)
	if err != nil {
		return err
	}

	typeFilename = fmt.Sprintf("%s_gen", typeFilename) + ".go"
